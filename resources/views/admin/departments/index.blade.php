<x-layouts.admin>
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <div>
            <h2 class="text-xl font-bold text-slate-800 leading-tight">ផ្នែក (Departments)</h2>
            <p class="text-sm text-slate-500 mt-1">គ្រប់គ្រងរចនាសម្ព័ន្ធអង្គភាព និងផ្នែក។ (Manage organization structure and divisions.)</p>
        </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-50 border-b border-slate-200 text-xs uppercase tracking-wider text-slate-500 font-semibold">
                        <th class="py-3 px-4">ឈ្មោះ (Name)</th>
                ...
                @forelse($departments as $department)
                    <tr class="hover:bg-slate-50 transition-colors">
                        ...
                        <td class="py-3 px-4 text-sm text-slate-600">
                            ...
                            @else
                                <span class="text-slate-400 italic">សកល (Global)</span>
                            @endif
                        </td>
                        <td class="py-3 px-4">
                        ...
                        <td class="py-3 px-4 text-right whitespace-nowrap">
                            <div class="flex items-center justify-end gap-2">
                                ...
                                <form method="POST" action="{{ route('admin.departments.destroy', $department) }}" class="inline-block" data-confirm="true">
                                    @csrf @method('DELETE')
                                    <button class="inline-flex items-center justify-center text-sm font-medium text-red-600 hover:text-red-800 bg-white border border-red-200 hover:bg-red-50 px-3 py-1.5 rounded-md transition-colors">លុប (Delete)</button>
                                </form>
                            </div>
                        </td>
                    </tr>
                @empty
                    <tr><td colspan="4" class="py-8 text-center text-slate-500">មិនបានរកឃើញផ្នែកណាមួយទេ (No departments found).</td></tr>
                @endforelse
                </tbody>
            </table>
        </div>
