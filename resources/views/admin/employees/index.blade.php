    ...
    <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
        ...
        <div class="sm:flex sm:justify-between sm:items-center mb-8">
            <div class="mb-4 sm:mb-0">
                ...
                <p class="text-slate-500 text-sm mt-1">គ្រប់គ្រងនិងមើលសមាជិកក្រុមរបស់អ្នក (Manage and view your team members).</p>
            </div>
        ...
        @if(session('status'))
            <div x-data="{ show: true }" x-show="show" class="mb-6 rounded-lg bg-green-50 p-4 text-sm text-green-800 border border-green-200 flex items-center shadow-sm">
                ...
                <div>
                    <span class="font-bold block">ជោគជ័យ (Success)</span>
                    {{ session('status') }}
                </div>
        ...
        <div class="bg-white rounded-xl shadow-[0_2px_10px_-3px_rgba(6,81,237,0.1)] border border-slate-100 mb-6 p-5">
            <h3 class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-4 flex items-center gap-2">
                ...
                ជម្រើសចម្រោះ (Filter Options)
            </h3>
            <form method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                <div>
                    <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5 ml-1">សាខា (Branch)</label>
                ...
                <div>
                    <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5 ml-1">ផ្នែក (Department)</label>
                    <div class="relative group">
                         <select name="department_id" class="w-full pl-3 pr-10 py-2.5 bg-slate-50 border border-slate-200 text-slate-700 text-sm rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 appearance-none transition-all cursor-pointer hover:bg-white hover:border-slate-300">
                            <option value="">គ្រប់ផ្នែក (All Departments)</option>
                ...
                <div>
                    <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5 ml-1">ស្ថានភាព (Status)</label>
                    <div class="relative group">
                        <select name="employment_status" class="w-full pl-3 pr-10 py-2.5 bg-slate-50 border border-slate-200 text-slate-700 text-sm rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 appearance-none transition-all cursor-pointer hover:bg-white hover:border-slate-300">
                            ...
                            <option value="active" @selected(request('employment_status')==='active')>សកម្ម (Active)</option>
                            <option value="suspended" @selected(request('employment_status')==='suspended')>ព្យួរ (Suspended)</option>
                            <option value="resigned" @selected(request('employment_status')==='resigned')>លាឈប់ (Resigned)</option>
                        </select>
                         <div class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-3 text-slate-400 group-hover:text-blue-500 transition-colors">
                            <i class="fa-solid fa-chevron-down text-xs"></i>
                        </div>
                ...
                <div class="flex gap-3">
                    ...
                    <a href="{{ route('admin.employees.index') }}" class="flex-none bg-white border border-slate-200 hover:bg-slate-50 text-slate-600 hover:text-slate-800 text-sm font-medium rounded-lg px-4 py-2.5 transition-colors shadow-sm text-center flex items-center justify-center tooltip" title="កំណត់ជម្រើសឡើងវិញ (Reset Filters)">
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-slate-100">
                    <thead class="bg-slate-50">
                        <tr>
                            <th scope="col" class="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-widest pl-6">បុគ្គលិក (Employee)</th>
                            <th scope="col" class="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-widest hidden sm:table-cell">ផ្នែក & សាខា (Dept & Branch)</th>
                            <th scope="col" class="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-widest hidden md:table-cell">ស្ថានភាព (Status)</th>
                            <th scope="col" class="px-6 py-4 text-right text-xs font-bold text-slate-500 uppercase tracking-widest pr-6">សកម្មភាព (Actions)</th>
                        </tr>
                    </thead>
                            ...
                            <tr>
                                <td colspan="4" class="px-6 py-16 text-center">
                                    <div class="flex flex-col items-center justify-center">
                                        ...
                                        <p class="text-slate-800 font-semibold text-lg">មិនមានបុគ្គលិកស្វែងរកឃើញទេ (No employees found)</p>
                                        <p class="text-slate-500 text-sm mt-1 max-w-xs mx-auto">គ្មានសមាជិកក្រុមណាមួយផ្គូផ្គងនឹងការចម្រោះបច្ចុប្បន្នទេ។ សូមព្យាយាមកំណត់ឡើងវិញ ឬបន្ថែមបុគ្គលិកថ្មី។</p>
                                        <a href="{{ route('admin.employees.create') }}" class="mt-4 text-blue-600 font-medium hover:text-blue-700 text-sm inline-flex items-center">
                                            <i class="fa-solid fa-plus mr-1"></i> បន្ថែមបុគ្គលិកថ្មី (Add New Employee)
                                        </a>
                                    </div>
                                </td>
                            </tr>
